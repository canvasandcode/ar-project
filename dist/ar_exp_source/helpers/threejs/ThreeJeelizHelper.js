...
THREE.JeelizHelper = (function() {
  ...
  function animate(t) {
    ...
    var amortizationCoeff = Math.pow(
    ...
    if (_ThreeMorphAnimMesh.material.type === "ShaderMaterial") {
      _ThreeMorphAnimMesh.material.set_rotationAmortized(
        _ThreeMorphAnimMesh.position,
        _ThreeMorphAnimMesh.scale,
        _rotationAmortized
      );
    }
  ...
  function load_model(url, mat, callback) {
    if (url === _loading.modelURL && mat === _loading.threeMat) {
      //called 2 times with same parameters
      return true;
    }
    ...
    ThreeMorphAnimGeomBuilder({
      ...
      successCallback: function(geom) {
        ...
        JEEFACETRANSFERAPI.on_detect(function(faceDetect) {
          _isFaceDetected = faceDetect;
          // faceDetect();
        });
  ...
  const that = {
    ...
    init: function(spec) {
      ...
      JEEFACETRANSFERAPI.init({
        ...
        callbackReady: function(errCode) {
          ...
          function start() {
            that.set_positionScale(
              spec.position ? spec.position : [0, 0, 0],
    ...
    set_materialParameters: function(params) {
      ...
      var matParameters = Object.assign(
        {
          ...
          morphJeelizInfluences:
            _ThreeMorphAnimMesh.userData.morphJeelizInfluences
        },
        params
      );
      [
      ...
      var mat = ThreeMorphFlexibleMaterialBuilder(
        matParameters,
